...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const updatePaymentMethodForm = document.getElementById("accountUpdateForm");
    updatePaymentMethodForm.addEventListener("submit", function(evt) {
        let valid = true;
        let errorP = document.getElementById("error");

        let username = document.getElementById("username");
        if(!username.value.match(new RegExp("^[a-zA-Z0-9]{8,}$")) && username.value != "") {
        ...
        if(!password.value.match(new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d]{8,}$")) && password.value != "") {
